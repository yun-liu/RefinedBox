...
def get_solvers(net_name):
    n = 'refined_box_alt_opt'
    # Solver for each training stage
    solvers = [[net_name, n, 'stage1_rerank_solver.prototxt'],
               [net_name, n, 'stage1_detection_solver.prototxt'],
               [net_name, n, 'stage2_rerank_solver.prototxt'],
               [net_name, n, 'stage2_detection_solver.prototxt']]
    solvers = [os.path.join(cfg.MODELS_DIR, *s) for s in solvers]
    # Iterations for each training stage
    if 'coco' not in cfg.MODELS_DIR:
        max_iters = [80000, 70000, 80000, 70000]
    else:
        max_iters = [1200000, 1200000, 1200000, 1200000]
    # max_iters = [200, 200, 200, 200]
    # Test prototxt for the RERANK
    rerank_test_prototxt = os.path.join(
        cfg.MODELS_DIR, net_name, n, 'rerank_test.prototxt')
    return solvers, max_iters, rerank_test_prototxt
...
def train_rerank(queue=None, imdb_names=None, init_model=None, solver=None,
    ...
    imdb, roidb = combined_roidb(imdb_names, mat_folder=cfg.TRAIN.RERANK_PROP_PATH)
...
def rerank_generate(queue=None, imdb_names=None, rerank_model_path=None, cfg=None,
    ...
    for imdb_name in imdb_names.split('+'):
        ...
        imdb.config['mat_file'] = os.path.join(cfg.DATA_DIR, cfg.TRAIN.RERANK_PROP_PATH,
                                        imdb_name + '.mat')
    ...
    rerank_proposals_path = os.path.join(output_dir, rerank_net_name + '_')
    queue.put({'proposal_path': rerank_proposals_path})

def train_detection(queue=None, imdb_names=None, init_model=None, solver=None,
                    max_iters=None, cfg=None, pkl_folder=None):
...
if __name__ == '__main__':
    ...
    if 'coco' in args.imdb_name:
        cfg.MODELS_DIR = osp.abspath(osp.join(cfg.ROOT_DIR, 'models', 'coco'))
    ...
    solvers, max_iters, rerank_test_prototxt = get_solvers(args.net_name)

    print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
    print 'Stage 1 RERANK, init from ImageNet model'
    print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
    ...
    mp_kwargs = dict(
            ...
            cfg=cfg)
    p = mp.Process(target=train_rerank, kwargs=mp_kwargs)
    p.start()
    rerank_stage1_out = mp_queue.get()
    p.join()

    print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
    print 'Stage 1 RERANK, generate proposals'
    print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'

    mp_kwargs = dict(
            queue=mp_queue,
            imdb_names=args.imdb_name,
            rerank_model_path=str(rerank_stage1_out['model_path']),
            cfg=cfg,
            rerank_test_prototxt=rerank_test_prototxt)
    p = mp.Process(target=rerank_generate, kwargs=mp_kwargs)
    p.start()
    rerank_stage1_out['proposal_path'] = mp_queue.get()['proposal_path']
    p.join()

    print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
    print 'Stage 1 detection net using reranked proposals, init from ImageNet model'
    print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
    ...
    mp_kwargs = dict(
            ...
            cfg=cfg,
            pkl_folder=rerank_stage1_out['proposal_path'])
    p = mp.Process(target=train_detection, kwargs=mp_kwargs)
    p.start()
    detection_stage1_out = mp_queue.get()
    p.join()

    print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
    print 'Stage 2 RERANK, init from stage 1 detection model'
    print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
    ...
    mp_kwargs = dict(
            queue=mp_queue,
            imdb_names=args.imdb_name,
            init_model=str(detection_stage1_out['model_path']),
            solver=solvers[2],
            max_iters=max_iters[2],
            cfg=cfg)
    p = mp.Process(target=train_rerank, kwargs=mp_kwargs)
    p.start()
    rerank_stage2_out = mp_queue.get()
    p.join()

    print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
    print 'Stage 2 RERANK, generate proposals'
    print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'

    mp_kwargs = dict(
            queue=mp_queue,
            imdb_names=args.imdb_name,
            rerank_model_path=str(rerank_stage2_out['model_path']),
            cfg=cfg,
            rerank_test_prototxt=rerank_test_prototxt)
    p = mp.Process(target=rerank_generate, kwargs=mp_kwargs)
    p.start()
    rerank_stage2_out['proposal_path'] = mp_queue.get()['proposal_path']
    p.join()

    print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
    print 'Stage 2 detection, init from stage 2 RERANK detection model'
    print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
    ...
    mp_kwargs = dict(
            queue=mp_queue,
            imdb_names=args.imdb_name,
            init_model=str(rerank_stage2_out['model_path']),
            solver=solvers[3],
            max_iters=max_iters[3],
            cfg=cfg,
            pkl_folder=rerank_stage2_out['proposal_path'])
    p = mp.Process(target=train_detection, kwargs=mp_kwargs)
    p.start()
    detection_stage2_out = mp_queue.get()
    p.join()

    # Create final model (just a copy of the last stage)
    final_path = os.path.join(
            os.path.dirname(detection_stage2_out['model_path']),
            args.net_name + '_refined_box_final.caffemodel')
    print 'cp {} -> {}'.format(
            detection_stage2_out['model_path'], final_path)
    shutil.copy(detection_stage2_out['model_path'], final_path)
